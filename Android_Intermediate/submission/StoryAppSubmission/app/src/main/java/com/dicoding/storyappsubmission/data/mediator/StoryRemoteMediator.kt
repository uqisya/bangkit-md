package com.dicoding.storyappsubmission.data.mediator
...
import com.dicoding.storyappsubmission.data.database.RemoteKeys
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, ListStoryItem>): MediatorResult {
        ...
        try {
            ...
            database.withTransaction {
                ...
                val keys = responseData.listStory.map { listStory ->
                    RemoteKeys(id = listStory.id, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                database.storyDao().insertListStory(responseData.listStory)
            }
